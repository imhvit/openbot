...
export default {
  ...
  async execute(message: Message, client: ExtendedClient) {
    ...
    try {
      if (command.developer && !config.developers.includes(message.author.id)) {
      ...
      if (command.permissions) {
        if (command.permissions.bot) {
          ...
          if (missingPerms?.length) {
            return message.reply(`Necesito los siguientes permisos: ${missingPerms.join(', ')}`);
          }
        }

        if (command.permissions.user) {
          const missingPerms = message.member?.permissions.missing(command.permissions.user);
          if (missingPerms?.length) {
            return message.reply(`Necesitas los siguientes permisos: ${missingPerms.join(', ')}`);
          }
        }
      }
          ...
          if (now < expirationTime) {
            ...
            setTimeout(async () => {
              try {
                ...
                console.log(
                  chalk.red(`[ERROR] `) + chalk.gray(`No se pudo eliminar el mensaje de cooldown`),
                );
              }
            }, 5000);
            return;
